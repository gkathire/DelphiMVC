...
type
  ...
  protected
    ...
    /// must be overriden, since we provide a fast SQLGetField() method
    function ColumnTypeNativeToDB(const aNativeType: RawUTF8; aScale: integer): TSQLDBFieldType; override;
  public
  ...
  protected
    ...
    /// get all field/column names for a specified MS SQL Server Table
    function SQLGetField(const aTableName: RawUTF8): RawUTF8; override;
    /// get all MS SQL Server table names
    function SQLGetTableNames: RawUTF8; override;
  public
  end;
...
function TOleDBStatement.ColumnType(Col: integer): TSQLDBFieldType;
begin
  CheckCol(Col);
...
begin
  ...
  if V=nil then // column is NULL
    ...
    ftDouble:
      if V^.Int64=0 then
        result := '0' else
        result := DoubleToStr(V^.Double,15);
    end;
end;
...
begin
  ...
  if V=nil then // column is NULL
    ...
    ftDouble:
      if V^.Int64=0 then
        result := '0' else
        result := DoubleToString(V^.Double,15);
...
function TOleDBOracleConnectionProperties.ColumnTypeNativeToDB(
  const aNativeType: RawUTF8; aScale: integer): TSQLDBFieldType;
begin
  result := OracleColumnTypeNativeToDB(aNativeType,aScale);
  if result=ftUnknown then
    // was not retrieved from OracleSQLGetField -> retrieve from OleDB type
...
function TOleDBOracleConnectionProperties.SQLGetTableNames: RawUTF8;
begin
  result := OracleSQLGetTableNames;
end;

function TOleDBOracleConnectionProperties.SQLIso8601ToDate(
...
function TOleDBMSSQLConnectionProperties.SQLGetField(const aTableName: RawUTF8): RawUTF8;
...
function TOleDBMSSQLConnectionProperties.SQLGetTableNames: RawUTF8;
begin
  result := 'select TABLE_SCHEMA + ''.'' + TABLE_NAME Name '+
    'from INFORMATION_SCHEMA.TABLES order by Name';
end;
