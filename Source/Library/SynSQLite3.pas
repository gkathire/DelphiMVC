...
{ ************ direct access to sqlite3.c / sqlite3.obj consts and functions }
{
 ...
 - FastCall use must be set with undefining USEC above, and
...
{$A4} // bcc32 default alignment is 4 bytes
  ...
  TSQLPager = record
    pVfs: pointer;
    exclusiveMode, journalMode, useJournal, noReadlock, noSync, fullSync,
...
begin
  ...
  if not isSelect(pointer(aSQL)) then begin // UPDATE, INSERT or any non SELECT
    if InternalState<>nil then
      inc(InternalState^);
    fCache.Reset; // cache must be flushed
...
begin
  ...
  end else begin // UPDATE, INSERT or any non SELECT statement
    if InternalState<>nil then
      inc(InternalState^);
    fCache.Reset; // flush the cache
    fLog.Log(sllCache,'cache flushed',self);
    result := '';
  end;
end;
...
begin
  try
    Execute(FormatUTF8('PRAGMA user_version=%;',[Value]));
  except
    on Exception do
      ;
  end;
...
begin
  if (self=nil) or (aFunction=nil) then
    exit;
  for i := 0 to fSQLFunctions.Count-1 do
    if IdemPropNameU(TSQLDataBaseSQLFunction(fSQLFunctions.List[i]).FunctionName,aFunction.FunctionName) then begin
      aFunction.Free;
...
begin
  ...
  result := sqlite3_prepare_v2(RequestDB, pointer(SQL), -1, fRequest, fNextSQL);
  while (result=SQLITE_OK) and (fNextSQL^<>#0) do // comment or white-space
...
begin
  if Request=0 then
    raise ESQLException.Create(RequestDB,SQLITE_MISUSE);
  result := sqlite3_check(RequestDB,sqlite3_reset(Request));
end;
...
begin
  ...
  for i := 0 to FieldCount-1 do begin
    ...
    case sqlite3_column_type(Request,i) of // fast evaluation: type may vary
      ...
      SQLITE_FLOAT:
        WR.Add(sqlite3_column_double(Request,i),15);
      SQLITE_TEXT: begin
        WR.Add('"');
