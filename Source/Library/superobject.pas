...
{$IFDEF HAVE_RTTI}
  ...
  public
    ...
    function AsType<T>(const obj: ISuperObject): T;overload;
    function AsType(const obj: ISuperObject; tp:PTypeInfo): TValue;overload;
...
{$ELSE}
  {$IF CompilerVersion > 22.0}WinApi.Windows {$ELSE} Windows{$IFEND}
{$ENDIF}
{$IFDEF FPC}
  ,sockets
{$ELSE}

{$IF CompilerVersion > 22.0},WinApi.WinSock {$ELSE} ,WinSock{$IFEND}
{$ENDIF};
...
function TSuperRttiContext.AsType(const obj: ISuperObject; tp:PTypeInfo): TValue;
begin
  FromJson(tp, obj, result);
end;
      ...
      stObject:
        begin
          ...
          for p in Context.GetType(Value.AsObject.ClassType).GetProperties do
            ...
            begin
              v := TValue.Empty;
              Result := FromJson(p.PropertyType.Handle, obj.AsObject[p.Name], v);
              if Result then
