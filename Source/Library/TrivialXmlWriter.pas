...
uses
  Classes, Generics.Collections,RTTI;
...
type
  ...
  public
    ...
    procedure WriteObjectRtti (AnObj: TValue);
...
procedure TTrivialXmlWriter.WriteObjectRtti(AnObj: TValue);
var
  aContext: TRttiContext;
  aType: TRttiType;
...
begin
  WriteString (sLineBreak); // new line
  aType := aContext.GetType(anObj.TypeInfo);
  for aField in aType.GetProperties do
  begin
    if aField.PropertyType.IsInstance then
    begin
      WriteStartElement (aField.Name);
      WriteObjectRtti (
        aField.GetValue(anObj.AsObject).AsObject);
      WriteEndElement (True);
    end
    else
    begin
      WriteStartElement (aField.Name);
      WriteString (aField.GetValue(anObj.AsObject).ToString);
      WriteEndElement;
    end;
  end;
end;
